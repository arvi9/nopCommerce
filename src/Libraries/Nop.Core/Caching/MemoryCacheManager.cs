...
namespace Nop.Core.Caching
{
    ...
    public partial class MemoryCacheManager : ILocker, IStaticCacheManager
    {
        ...
        public T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            return _provider.Get(key, acquire, TimeSpan.FromSeconds(cacheTime ?? NopCachingDefaults.CacheTime))
                .Value;
        }
        ...
        {
            if (!(cacheTime <= 0))
                return await _provider.GetAsync(key, acquire, TimeSpan.FromSeconds(cacheTime ?? NopCachingDefaults.CacheTime));
        ...
        public void RemoveByPattern(string prefix)
        {
